...
- [x] **17.6** Tests in `tests/test_review.py` + `tests/test_srs.py`: SM-2 progression (`again` lapses + bumps lapses; consecutive `good` grows interval; `ease_factor` floor 1.3); queue returns due, non-suspended cards for the right user+language then new cards; grading moves `due_at` into the future; auth required (returns 403 via `HTTPBearer`, the app-wide convention, not 401); a user can't grade another user's card (404/403). _(Pure-SM-2 day-interval assertions were rewritten for the learning-steps model in 17.8.)_
- [x] **17.7** Verify: register → save a word → `GET /api/review/queue` returns it → `POST` a `good` grade → it leaves the queue and `due_at` is ~1 day out; `docs/API.md` updated with the new endpoints. _(Verified via the frontend; `docs/API.md` now has the Review section.)_
- [x] **17.8** Anki-style **learning steps** in `app/services/srs.py` (enhancement beyond plain SM-2): cards walk sub-day steps `LEARNING_STEPS_MINUTES = (1, 10)` (state flagged by `interval_days == 0`, `due_at` carries the sub-day time — no schema change). `again` restarts at step 0 so a forgotten card resurfaces the same session; `good` graduates to 1 day, `easy` to 4 days. In the review phase `again` is a true lapse (ease −0.20, `lapses++`, back to relearning). _Frontend must refetch `/queue` for re-lapsed cards to reappear._
- [x] **17.9** Rolling, client-configurable **daily new-card cap**: `GET /api/review/queue?new_per_day=N` (default 20) introduces at most `N − (new cards first-reviewed today, UTC day)` brand-new cards. Backed by a new nullable `first_reviewed_at` column (migration `0009`), stamped on a card's first-ever grade so later lapses don't reset the day's count.
